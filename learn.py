...
def train(Theta1,Theta2,X,y,_lambda,num_labels):
    m = np.size(X,0)
    a1 = X
    ...
    Theta2 = np.dot(delta3.getT(),a2) / m
    Theta1 = np.dot(delta2,a1) / m
    for i in range(np.size(Theta1NoBias,0)):
        for j in range(1,np.size(Theta1NoBias,1)):
            Theta1.itemset((i,j), Theta1.item((i,j)) + (Theta1NoBias.item((i,j)) * _lambda / m))
    for i in range(np.size(Theta2NoBias,0)):
        for j in range(1,np.size(Theta2NoBias,1)):
            Theta2.itemset((i,j), Theta2.item((i,j)) + (Theta2NoBias.item((i,j)) * _lambda / m))
...
y = y_tmp
for i in range(100):
    theta1, theta2 = train(theta1,theta2,X,y,1,num_labels)
